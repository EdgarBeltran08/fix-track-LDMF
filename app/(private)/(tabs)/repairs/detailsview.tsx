    ...
    >
      ...
      <View className="bg-background-100 border border-background-200 rounded-2xl p-5 mb-6">
        ...
        {piezas.map((p) => (
          ...
          >
            <View>
              <Text className="text-typography-900 font-semibold">{p.name}</Text>
              <Text className="text-typography-900 text-sm">
                Cantidad: {p.quantity}
              </Text>
            </View>
            <Text className="text-primary-600 font-bold">${p.cost.toFixed(2)}</Text>
          </View>
        ))}
      </View>
      ...
      <View className="bg-background-100 border border-background-200 rounded-2xl p-5 mb-6">
        ...
        <View className="flex-row justify-between mb-2">
          <Text className="text-typography-900 font-semibold">Mano de obra</Text>
          <Text className="text-primary-600 font-bold">${laborCost.toFixed(2)}</Text>
        </View>
        <View className="flex-row justify-between mb-2">
          <Text className="text-typography-900 font-semibold">Piezas</Text>
          <Text className="text-primary-600 font-bold">${partsCost.toFixed(2)}</Text>
        </View>
        <View className="flex-row justify-between border-t border-background-200 pt-2">
          ...
          <Text className="text-primary-600 font-bold">${totalCost.toFixed(2)}</Text>
        </View>
      </View>

      {/* NOTAS */}
      <View className="bg-background-100 border border-background-200 rounded-2xl p-5 mb-6">
        <Text className="text-xl font-bold text-typography-900 mb-3">Notas</Text>
