...
import { Modal, ModalBackdrop, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader } from '@/shared/components/ui/modal';
...
const InventoryPage: React.FC = () => {
  ...
  return (
    <View className="flex-1 pt-5 mb-8 items-start bg-background-50">
      <Text className="text-3xl font-bold ml-6 text-secondary-900">Inventario</Text>

      {/* Barra de busqueda */}
      <View className="my-2 w-3/5 ml-5">
        <Input className="border-2 border-secondary-300 rounded-xl">
      ...
      <View className="flex-1 pt-2 ml-5 mx-2 w-11/12 max-h-[550px] border-b-2 border-primary rounded-xl">
        <ScrollView 
          className="flex-1"
        ...
        >
          <ScrollView 
            ...
            className="w-full"
          >
            <Table className="min-w-[600px] border-2 border-secondary-300 rounded-xl overflow-visible">
              <TableHeader>
                <TableRow className="bg-background-50">
                  <TableHead className="text-secondary-900 px-2">Repuesto</TableHead>
                  <TableHead className="text-secondary-900 px-2">Cantidad</TableHead>
                  <TableHead className="text-secondary-900 px-2">ID</TableHead>
                  <TableHead className="text-secondary-900 px-2">Categoría</TableHead>
                  <TableHead className="text-secondary-900 px-2">Estado</TableHead>
                  <TableHead className="text-secondary-900 px-2">Detalle</TableHead>
                </TableRow>
              </TableHeader>
              ...
              <TableBody>
                {filteredData.map((item) => (
                  <TableRow key={item.id} className="bg-background-50">
                    <TableData className="px-2 text-secondary-900  border-secondary-300">{item.repuesto}</TableData>
                    <TableData className="px-2 text-secondary-900  border-secondary-300">{item.cantidad}</TableData>
                    <TableData className="px-2 text-secondary-900  border-secondary-300">{item.idRepuesto}</TableData>
                    <TableData className="px-2 text-secondary-900 border-secondary-300">{item.categoria}</TableData>
                    <TableData className="px-2 text-secondary-900  border-secondary-300">{item.estado}</TableData>
                    <TableData className="px-2 text-secondary-900  border-secondary-300">
                      ...
                      <TouchableOpacity onPress={() => handleClickVer(item)}>
                        <Text className="ml-6 text-blue-600 underline text-base font-bold">Ver</Text> 
                      </TouchableOpacity>
                    </TableData>
                  </TableRow>
                ))}
              ...
              <TableFooter>
                <TableRow className="bg-secondary-200">
                  <TableHead className="text-secondary-900 px-2">Total Items</TableHead>
                  <TableHead className="text-secondary-900 px-2">{totalItems}</TableHead>
